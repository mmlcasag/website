CREATE OR REPLACE PROCEDURE website.PROC_GRAVA_PEDIDO_SITE( -- PEDIDO
  ...
  begin
    ...
    values
      (
       ...
       'LOJA VIRTUAL - PROC_GRAVA_PEDIDO_SITE,' ||
    ...
    if pcodfil = 400 then
      v_sql := ' insert into log_prog_alt_pedidos
                 ...
                 values
                   ( ' || pnumpedven || ', ''PROC_GRAVA_PEDIDO_SITE'', ''MOV_PEDIDO'', systimestamp, ' || nvl(padmcartao,0) || ')';
          ...
          begin
            ...
            if pcodfil = 400 then
              v_sql := ' insert into log_prog_alt_pedidos
                           ( pedido, programa, tabela, data, valor )
                         values
                           ( ' || wnumpedvengar || ', ''PROC_GRAVA_PEDIDO_SITE'', ''MOV_PEDIDO'', systimestamp, ' || nvl(padmcartao,0) || ')';
...
';
  ...
  send_mail('site-erros@colombo.com.br','site-erros@colombo.com.br','PROC_GRAVA_PEDIDO_SITE','Erro: ' || sqlerrm || ' na linha: ' || DBMS_UTILITY.FORMAT_ERROR_BACKTRACE || ' Debug: ' || v_mensagem);
end;
  
  pmens := pmens || ' ERRO: ' || sqlerrm || ' na linha: ' || DBMS_UTILITY.FORMAT_ERROR_BACKTRACE;
  raise_application_error(-20903, pmens);
  
end proc_grava_pedido_site;
/
